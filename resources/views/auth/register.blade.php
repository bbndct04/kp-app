...
<head>
    ...
    <style>
        ...
        .reg-step-side.active .reg-step-num {
            background:var(--blue-500); border-color:var(--blue-400);
            color:#fff; box-shadow:0 0 0 4px rgba(43,126,237,.25);
        }
        .reg-step-side.done .reg-step-num {
            background:rgba(13,122,78,.7); border-color:rgba(13,122,78,.9); color:#fff;
        }
        ...
        .reg-step-line { width:2px; height:28px; background:rgba(255,255,255,.12); margin:4px 0 4px 15px; border-radius:2px; }

        /* Upload zone */
        .upload-zone-reg {
            border:2px dashed #d4daea; border-radius:14px;
            padding:32px 24px; text-align:center; cursor:pointer;
            transition:all .2s; background:#f8f9ff; min-height:180px;
            display:flex; align-items:center; justify-content:center;
        }
...
<body style="background:#f0f4fc;font-family:Inter,sans-serif;min-height:100vh;">

<div style="display:grid;grid-template-columns:400px 1fr;min-height:100vh;">
    ...
    <div class="auth-panel">
        <div style="position:relative;z-index:1;text-align:center;width:100%;">
            ...
            <div style="display:flex;justify-content:center;margin-bottom:24px;">
                <svg width="130" height="143" viewBox="0 0 200 220" fill="none"
            {{-- Dual Logos --}}
            <div style="display:flex;align-items:center;justify-content:center;gap:18px;margin-bottom:20px;">
                <div style="width:78px;height:78px;border-radius:50%;overflow:hidden;border:3px solid rgba(255,255,255,.35);flex-shrink:0;animation:bounceReg 2.8s ease-in-out 1.2s infinite;">
                    <img src="{{ asset('images/blotterlink-logo.png') }}" alt="BlotterLink"
                        style="width:100%;height:100%;object-fit:cover;display:block;">
                </div>
                <div style="width:1.5px;height:52px;background:rgba(255,255,255,.2);border-radius:99px;flex-shrink:0;"></div>
                <div style="width:78px;height:78px;border-radius:50%;overflow:hidden;border:3px solid rgba(255,255,255,.35);flex-shrink:0;background:#fff;animation:bounceReg 2.8s ease-in-out 1.5s infinite;">
                    <img src="{{ asset('images/barangay-logo.jpg') }}" alt="Barangay New Kababae"
                        style="width:100%;height:100%;object-fit:cover;display:block;">
                </div>
            </div>

            <h1 style="font-size:26px;font-weight:700;color:#fff;letter-spacing:-.5px;margin-bottom:6px;">BlotterLink</h1>
            <div style="width:36px;height:2.5px;background:rgba(91,160,245,.8);margin:0 auto 10px;border-radius:99px;"></div>
            <p style="font-size:13.5px;color:rgba(255,255,255,.6);margin-bottom:6px;line-height:1.6;">Barangay Complaints &amp;<br>Incident Reporting System</p>

            {{-- Step Progress --}}
            <div style="text-align:left;margin-top:8px;" id="step-sidebar">
                <div class="reg-step-side active" id="side-1">
                    <div class="reg-step-num" id="snum-1">1</div>
                    <div><div class="reg-step-label">ID Verification</div><div class="reg-step-sub">Upload your valid ID</div></div>
                </div>
                <div class="reg-step-line"></div>
                <div class="reg-step-side" id="side-2">
                    <div class="reg-step-num" id="snum-2">2</div>
                    <div><div class="reg-step-label">Face Photo</div><div class="reg-step-sub">Take or upload a selfie</div></div>
                </div>
                <div class="reg-step-line"></div>
                <div class="reg-step-side" id="side-3">
                    <div class="reg-step-num" id="snum-3">3</div>
                    <div><div class="reg-step-label">Your Details</div><div class="reg-step-sub">Review &amp; complete info</div></div>
                </div>
                <div class="reg-step-line"></div>
                <div class="reg-step-side" id="side-4">
                    <div class="reg-step-num" id="snum-4">4</div>
                    <div><div class="reg-step-label">Terms &amp; Account</div><div class="reg-step-sub">Agree and set password</div></div>
                </div>
            </div>
    ...
    {{-- RIGHT FORM SIDE --}}
    <div style="background:#f0f4fc;display:flex;align-items:flex-start;justify-content:center;padding:40px 52px;overflow-y:auto;min-height:100vh;">
            ...
            <form method="POST" action="{{ route('register') }}" id="reg-form" enctype="multipart/form-data">
                ...
                <div id="step-1">
                    ...
                    <div style="margin-top:18px;display:flex;justify-content:space-between;align-items:center;">
                        <button type="button" onclick="skipID()"
                            style="background:none;border:1.5px solid #d4daea;border-radius:8px;padding:10px 20px;font-size:14px;font-weight:500;color:#5e6882;cursor:pointer;font-family:Inter,sans-serif;">
                            Skip — Fill manually
                        </button>
                ...
                <div id="step-2" style="display:none;">
                    ...
                    <div style="margin-top:18px;display:flex;justify-content:space-between;">
                        <button type="button" onclick="goStep(1)" class="btn-ghost" style="width:auto;padding:10px 20px;">
                            &lsaquo; Back
                        </button>
                        <button type="button" id="btn-step2-next" onclick="goStep(3)" disabled
                            class="btn-primary" style="width:auto;padding:11px 28px;opacity:.5;cursor:not-allowed;">
                            Next — Review Details &rsaquo;
                        </button>
                    </div>
                </div>
                ...
                <div id="step-3" style="display:none;">
                    ...
                    <div style="display:flex;justify-content:space-between;margin-top:8px;">
                        <button type="button" onclick="goStep(2)" class="btn-ghost" style="width:auto;padding:10px 20px;">&lsaquo; Back</button>
                        <button type="button" onclick="validateStep3()" class="btn-primary" style="width:auto;padding:11px 28px;">
                            Next — Terms &amp; Password &rsaquo;
                        </button>
                    </div>
                </div>
                ...
                <div id="step-4" style="display:none;">
                    ...
                    <div style="margin-top:14px;">
                        <button type="button" onclick="goStep(3)" class="btn-ghost" style="width:auto;padding:10px 20px;">&lsaquo; Back</button>
                    </div>
                </div>
...
async function handleIDUpload(input) {
    ...
    reader.onload = async (e) => {
        ...
        try {
            ...
            applyIDData(data);
            // Store face image from ID for later face match
...
async function scanID(base64, mime) {
    ...
    // Map ID Analyzer response to our format
    const r = d.result || {};
    return {
...
// ─── Face Verification ───
async function verifyFace(faceBase64, idFaceBase64) {
    if (!idFaceBase64) return null; // skip if no face from ID
    ...
    const d = await res.json();
    return d.confidence || null; // returns 0-1 confidence score
}

function formatDOB(dob) {
    // ID Analyzer returns YYYY/MM/DD or YYYY-MM-DD
    if (!dob) return '';
...
function applyIDData(data) {
    ...
    if (data.sex) {
        ...
        s.value = data.sex; s.classList.add('autofilled');
...
// ─── Face Upload ───
let idFaceBase64 = null; // stored from ID scan result
let faceBase64   = null; // stored from selfie upload
...
function handleFaceUpload(input) {
    ...
    reader.onload = async (e) => {
        ...
        // Try face match if we have ID face
        if (idFaceBase64) {
...
function checkScroll() {
    ...
    if (atBottom && !state.termsRead) {
        ...
        const note = document.getElementById('terms-lock-note');
        box.classList.remove('disabled');
        hint.className = 'terms-hint unlocked';
        hint.innerHTML = '✓ You\'ve read the terms — click below to agree';
...
// ─── GSAP entrance ───
gsap.from('.auth-panel > div', { duration:.8, x:-30, opacity:0, ease:'power2.out' });
